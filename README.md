...
- **Model**:
  ...
  - Использует `AppStateEmitter` для оповещения View и Controller об изменениях в данных.
...
- **Методы**:
  - `loadProductCatalog(): Promise<void>`: Получает через API каталог товаров и уведомляет об изменении.
  - `placeOrder(): Promise<void>`: Отправляет текущий заказ пользователя и уведомляет об изменении.
